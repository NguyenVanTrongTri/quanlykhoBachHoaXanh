...
if($_SERVER['REQUEST_METHOD'] == 'POST'&& isset($_POST['add_item'])){
    ...
    $maVach     = trim($_POST['mavach'] ?? '');

    // 2. Lưu tạm vào session ngay lập tức
    ...
    // 3. Lấy tên loại hàng nếu có
    $tenLoaiHang = null;
    if ($maLoaiHang !== '') {
        $stmt = $conn->prepare("SELECT TenLoaiHang FROM loaihanghoa WHERE MaLoaiHang = ?");
        $stmt->execute([$maLoaiHang]);
        $tenLoaiHang = $stmt->fetchColumn(); 
    }
    ...
    if ($check->fetch()) {
        ...
        $_SESSION['popup_message'] = 'Mã hàng hóa đã tồn tại trong hệ thống';
    ...
    $sql = "INSERT INTO hanghoa 
            (MaHangHoa, TenHangHoa, DonViTinh, MaLoaiHang,XuatXu, MaVach)
            VALUES (?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->execute([$maHangHoa, $tenHangHoa, $donViTinh, $maLoaiHang, $xuatXu, $maVach]);

    // 6. Thêm vào session chính
    $_SESSION['Hanghoa_items'][] = [
        'MaHangHoa'   => $maHangHoa,
        'TenHangHoa'  => $tenHangHoa,
        'DonViTinh'   => $donViTinh,
...
if ( $_SERVER['REQUEST_METHOD'] == 'POST'&& isset($_POST['delete_item']) && isset($_POST['delete_index'])) {
    ...
    if (!isset($_SESSION['Hanghoa_items'][$deleteIndex])) {
        $_SESSION['popup_error'] = true;
        $_SESSION['popup_message'] = 'Dòng cần xóa không tồn tại';
        header("Location: " . $_SERVER['PHP_SELF']);
        exit;
    }

    
    $maHangHoa = $_SESSION['Hanghoa_items'][$deleteIndex]['MaHangHoa'];
    try {
        $sql = "DELETE FROM hanghoa WHERE MaHangHoa = ?";
        $stmt = $conn->prepare($sql);
        $stmt->execute([$maHangHoa]);

        unset($_SESSION['Hanghoa_items'][$deleteIndex]);
        ...
        $_SESSION['popup_error'] = true;
        $_SESSION['popup_message'] = 'Lỗi xóa hàng hóa: ' . $e->getMessage();
        header("Location: " . $_SERVER['PHP_SELF']);
        exit;
    }
}
                ...
                <div class="section-title">Thông tin hàng hóa</div>
                    <div class="form-group">
                        <div class="form-field">
                            ...
                            <input type="text" id="mahanghoa" name="mahanghoa" value="<?= $hanghoa['MaHangHoa'] ?? '' ?>" autocomplete="off" placeholder="HH001">
                        </div>

                        <div class="form-field">
                        <label for="maloaihang">Mã loại hàng hóa</label>
                        <select id="maloaihang" name="maloaihang" autocomplete="off"  value="<?= $hanghoa['MaLoaiHang'] ?? '' ?>" class="form-select">
                        ...
                        <div class="form-field">
                            ...
                            <input type="text" id="tenhanghoa" name="tenhanghoa" value="<?= $hanghoa['TenHangHoa'] ?? '' ?>" placeholder="Dưa hấu không hạt">
                        </div>
                        <div class="form-field">
                            <label for="tenloaihanghoa">Tên loại hàng hóa</label>
                            <input type="text" id="tenloaihanghoa" name="tenloaihanghoa"   value="<?= $hanghoa['TenLoaiHang'] ?? '' ?>"  readonly>
                            <script>
                        ...
                        <select id="donvitinh" name="donvitinh" class="form-select">
                            <option value="" <?= empty($hanghoa['DonViTinh']) ? 'selected' : '' ?>>-- Chọn đơn vị --</option>
                            <option value="Kg" <?= (isset($hanghoa['DonViTinh']) && $hanghoa['DonViTinh'] === 'Kg') ? 'selected' : '' ?>>Kg</option>
                            <option value="Thùng" <?= (isset($hanghoa['DonViTinh']) && $hanghoa['DonViTinh'] === 'Thùng') ? 'selected' : '' ?>>Thùng</option>
                            <option value="Hộp" <?= (isset($hanghoa['DonViTinh']) && $hanghoa['DonViTinh'] === 'Hộp') ? 'selected' : '' ?>>Hộp</option>
                        </select>
                        </div>
                        <div class="form-field">
                            <label for="xuatxu">Xuất xứ</label>
                            <input type="text" id="xuatxu"  name="xuatxu" value="<?= $hanghoa['XuatXu'] ?? '' ?>" placeholder="">
                        </div>
                        <div class="form-field">
                            <label for="mavach">Mã vạch</label>
                            <input type="text" id="mavach"  name="mavach"placeholder="" value="<?= $hanghoa['MaVach'] ?? '' ?>">
                        </div>
                         <script>
                            //hàm thông báo
                            function closePopup() {
                            ...
                            document.getElementById("formHangHoa").onsubmit = function(e) {
                                ...
                                if (e.submitter && e.submitter.name === "add_item") {
                                    if (document.getElementById("mahanghoa").value.trim() === "") {
                                         e.preventDefault(); // CHẶN LỆNH GỬI FORM TẠI ĐÂY
                                        showPopup("Mã hàng hóa không được để trống");
                                        return false;
                                    }  
                                    if (document.getElementById("maloaihang").value.trim() === "") {
                                         e.preventDefault(); // CHẶN LỆNH GỬI FORM TẠI ĐÂY
                                        showPopup("Mã loại hàng hóa không được để trống");
                                        return false;
                                    }
                                    if (document.getElementById("tenhanghoa").value.trim() === "") {
                                         e.preventDefault(); // CHẶN LỆNH GỬI FORM TẠI ĐÂY
                                        showPopup("Tên hàng hóa không được để trống");
                                        return false;
                                    } 
                                    if (document.getElementById("donvitinh").value.trim() === "") {
                                         e.preventDefault(); // CHẶN LỆNH GỬI FORM TẠI ĐÂY
                                        showPopup("Đơn vị tính không được để trống");
                                        return false;
                                    }
                                    if (document.getElementById("xuatxu").value.trim() === "") {
                                         e.preventDefault(); // CHẶN LỆNH GỬI FORM TẠI ĐÂY
                                        showPopup("Xuất xứ không được để trống");
                                        return false;
                                    }
                                    
                                    if (document.getElementById("mavach").value.trim() === "") {
                                         e.preventDefault(); // CHẶN LỆNH GỬI FORM TẠI ĐÂY
                                        showPopup("Mã vạch không được để trống");
                                        return false;
                                    }
                    ...
                    <div class="action-buttons">
                        ...
                        <button type="submit" name="delete_cancel" class="btn delete"> Hủy bỏ</button>
                    </div>
            </div>
