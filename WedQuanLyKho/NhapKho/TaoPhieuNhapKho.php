...
function taoMaPhieuNhap($conn){

    $prefix = "PN026";

    // Lấy mã lớn nhất hiện tại
    $sql = "SELECT MaPhieuNhap 
            FROM phieunhap
    ...
    do {
        ...
        $checkSql = "SELECT COUNT(*) FROM phieunhap WHERE MaPhieuNhap = :code";
    ...
    } while ($exists > 0); // nếu trùng thì tăng tiếp

    return $newCode;
}
function laySucChua($conn, $maHangHoa){
    ...
    if($row){
        return $row['ConTrong']; // trả về sức chứa còn lại
    }

    return 0; // nếu không có kệ
}
$sqlHH = "
    SELECT DISTINCT 
        hh.MaHangHoa,
...
if (!isset($_SESSION['Nhapkho_items'])) {
    $_SESSION['Nhapkho_items'] = [];
}
if (!isset($_SESSION['PhieuNhap_items'])) {
    $_SESSION['PhieuNhap_items'] = [];
}
//Chức năng thêm
if($_SERVER['REQUEST_METHOD'] == 'POST'&& isset($_POST['add_item'])){
    ...
    $sl_tt = intval($_POST['soluongthuctenhap'] ?? 0);
    $dv = $_POST['donvitinh'];
    $dg = floatval($_POST['dongia'] ?? 0);
    $tt = $sl_tt * $dg; // tính thành tiền
    
    $stmt = $conn->prepare("
    SELECT MaKeHang, ConTrong
    FROM kehanghoa
    ...
    $_SESSION['popup_message'] = 
         "Hàng hóa chưa được gán kệ lưu trữ. Vui lòng liên hệ thủ kho để biết thêm chi tiết!";
    ...
    // KIỂM TRA TRÙNG HÀNG HÓA TRONG SESSION
    if (!empty($_SESSION['Nhapkho_items'])) {
        foreach ($_SESSION['Nhapkho_items'] as $item) {

            if ($item['MaHangHoa'] == $ma) {

                $_SESSION['popup_error'] = true;
                $_SESSION['popup_message'] =
                "Hàng hóa $ma đã tồn tại trong phiếu nhập, Bạn vui lòng xóa và nhập lại hàng hóa mới";
    ...
    $conTrong = (int)$ke['ConTrong'];

    if (!empty($_SESSION['Nhapkho_items'])) {
        foreach ($_SESSION['Nhapkho_items'] as $item) {
            if ($item['MaKeHang'] === $maKeHang) {
            $daNhapTam += $item['SoLuongThucTeNhap'];
            }
        }
    }
    
    $conTrongThuc = $conTrong - $daNhapTam;
    // 3. KIỂM TRA SỨC CHỨA KỆ
    if ($sl_tt > $conTrongThuc) {
    $_SESSION['popup_error'] = true;
    $_SESSION['popup_message'] =
        "Kệ hàng $maKeHang đã đầy, không thể thêm hàng hóa,Vui lòng liên hệ thủ kho để biết thêm chi tiết !";
    }
    // Thêm vào session
    $_SESSION['Nhapkho_items'][] = [
        'MaHangHoa' => $ma,
...
if ( $_SERVER['REQUEST_METHOD'] == 'POST'&& isset($_POST['delete_item']) && isset($_POST['delete_index'])) {
    ...
    unset($_SESSION['Nhapkho_items'][$index]);
    $_SESSION['Nhapkho_items'] = array_values($_SESSION['Nhapkho_items']);
}
//Chức năng LƯU
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['save'])) {
    try {
        ...
        $rowNV = $stmtNV->fetch(PDO::FETCH_ASSOC);
        ...
        // LẤY THÔNG TIN CHUNG PHIẾU NHẬP TỪ FORM
        $sqlCheck = "SELECT 1 FROM phieunhap WHERE MaPhieuNhap = :MaPhieuNhap LIMIT 1";
        $stmtCheck = $conn->prepare($sqlCheck);
        $stmtCheck->execute([
            ':MaPhieuNhap' => $soHoaDon
        ]);

        if ($stmtCheck->fetch()) {
            $_SESSION['popup_error'] = true;
            $_SESSION['popup_message'] =
                "Mã phiếu nhập $maPhieuNhap đã tồn tại trong hệ thống!";
        ...
        $MaDVVC="DV001";
        $ThoiGian="20/11/2004";
        $sqlPhieu = "INSERT INTO phieunhap
            (MaPhieuNhap, ThoiGian, MaNhanVien, MaNCC, DonVi,NguoiGiaoHang,DiaChi,NhapTaiKho,
             BoPhan, DiaDiem, MaDonViVanChuyen)
            VALUES
            (:MaPhieuNhap, :ThoiGian, :MaNhanVien, :MaNCC, :DonVi,:NguoiGiaoHang,:DiaChi,:NhapTaiKho,
             :BoPhan, :DiaDiem, :MaDonViVanChuyen )";
    
        $thoiGianInput = $phieu['ThoiGian'] ?? '19/12/2004 12:00 am';
        $thoiGianMySQL = convertToMySQLDateTime($thoiGianInput);
  
        $stmtPhieu = $conn->prepare($sqlPhieu);
        $stmtPhieu->execute([
            ':MaPhieuNhap'      => $phieu['SoHoaDon'],
            ':ThoiGian'         => $phieu['ThoiGian'],
            ':MaNhanVien'       => $maNV,
            ':MaNCC'            => $MaNCC,
            ':DonVi'            => $phieu['DonVi'],
            ':NguoiGiaoHang'    => $phieu['NguoiGiaoHang'],
            ':DiaChi'           => $phieu['DiaChi'],
            ':NhapTaiKho'       => $phieu['NhapTaiKho'],
            ':BoPhan'           => $phieu['BoPhan'],
            ':DiaDiem'          => $phieu['DiaDiem'],
            ':MaDonViVanChuyen' => $MaDVVC,
        ]);
    
        $sqlCT = "INSERT INTO chitietphieunhap
            (MaPhieuNhap, MaHangHoa, SoLuongTheoChungTu,
             SoLuongThucTeNhap, DonViTinh, DonGia)
            VALUES
            (:MaPhieuNhap, :MaHangHoa, :SoLuongTheoChungTu,
             :SoLuongThucTeNhap, :DonViTinh, :DonGia)";
        ...
        foreach ($_SESSION['Nhapkho_items'] as $item) {
            $stmtCT->execute([
                ...
                ':SoLuongThucTeNhap'  => $item['SoLuongThucTeNhap'],
                ':DonViTinh'          => $item['DonViTinh'],
                ':DonGia'             => $item['DonGia'],
            ]);
        }
        function getTonKhoByHangHoa($conn, $maHangHoa) {
            $sql = "SELECT * FROM tonkho WHERE MaHangHoa = :MaHangHoa LIMIT 1";
            $stmt = $conn->prepare($sql);
            $stmt->execute([':MaHangHoa' => $maHangHoa]);
            return $stmt->fetch(PDO::FETCH_ASSOC); // trả về mảng tồn kho hoặc false nếu chưa có
        }
        function generateMaTonKho($conn) {
                do {
                    $stmt = $conn->query("SELECT MAX(CAST(SUBSTRING(MaTonKho, 3) AS UNSIGNED)) AS maxSo FROM tonkho");
                    $row  = $stmt->fetch(PDO::FETCH_ASSOC);

                    $so = ($row['maxSo'] ?? 0) + 1;
                    // Sinh mã ngẫu nhiên (độ dài an toàn cho CHAR/VARCHAR)
                    $maTonKho = 'TK' . str_pad($so, 3, '0', STR_PAD_LEFT);

                    // Kiểm tra trùng
                    $stmt = $conn->prepare("SELECT 1 FROM tonkho WHERE MaTonKho = ?");
                    $stmt->execute([$maTonKho]);
        ...
        $sqlGetDanhMuc = "
            SELECT lh.MaDanhMuc
            FROM hanghoa hh
            INNER JOIN loaihanghoa lh
                ON hh.MaLoaiHang = lh.MaLoaiHang
            WHERE hh.MaHangHoa = :MaHangHoa
            ";
        function getMaDanhMucByHangHoa($conn, $maHangHoa) {
        $sql = "
            SELECT lh.MaDanhMuc
            FROM hanghoa hh
            INNER JOIN loaihanghoa lh
        ...
        return $stmt->fetchColumn(); // trả về MaDanhMuc
        }
        function getMaLoaiKhoByHangHoa($conn, $maHangHoa) {
        $sql = "
            SELECT lk.MaLoaiKho
            FROM hanghoa hh
            INNER JOIN kehanghoa kh
        ...
        $stmt = $conn->prepare($sql);
        $stmt->execute([':MaHangHoa' => $maHangHoa]);
        return $stmt->fetchColumn(); // trả về MaLoaiKho
        }   
        $sqlTonKho = "
        INSERT INTO tonkho
            (MaTonKho, MaDanhMuc, MaHangHoa, MaLoaiKho, TrangThai, GhiChu)
        SELECT
            :MaTonKho,
            :MaDanhMuc,
            ct.MaHangHoa,
        ...
        AND ct.MaHangHoa = :MaHangHoa
        ";
        $stmtTonKho = $conn->prepare($sqlTonKho);

        foreach ($_SESSION['Nhapkho_items'] as $item) {
        
        $maHangHoa = $item['MaHangHoa']; 
            $sqlGetKe = "
            SELECT MaKeHang, TongSucChua, DaChua
            FROM kehanghoa
            WHERE MaHangHoa = :MaHangHoa
            LIMIT 1
            ";
            $stmtGetKe = $conn->prepare($sqlGetKe);
            $stmtGetKe->execute([':MaHangHoa' => $maHangHoa]);
            $ke = $stmtGetKe->fetch(PDO::FETCH_ASSOC);

        $soLuongNhap = (int)$item['SoLuongThucTeNhap'];
        if ($soLuongNhap <= 0) continue;

        $tonKhoHienTai = getTonKhoByHangHoa($conn, $maHangHoa);
        $maTonKho = $tonKhoHienTai['MaTonKho']; 
        if($tonKhoHienTai && $ke){
                $sqlUpdateKe = "
                        UPDATE kehanghoa
                        SET DaChua = DaChua + :SoLuongNhap
                        WHERE MaKeHang = :MaKeHang
            ";

            $stmtUpdate = $conn->prepare($sqlUpdateKe);
            $stmtUpdate->execute([
            ...
            if (!$ke) {
                throw new Exception("Chưa cấu hình kệ cho hàng hóa {$item['MaHangHoa']}");
            }

            if ($soLuongNhap > $ke['TongSucChua']) {
                throw new Exception("Kệ {$ke['MaKeHang']} không đủ sức chứa");
            }

            // 1️⃣ INSERT TỒN KHO
            $maTonKho = generateMaTonKho($conn);
            $maDanhMuc = getMaDanhMucByHangHoa($conn, $maHangHoa);
            $maLoaiKho = getMaLoaiKhoByHangHoa($conn, $maHangHoa);
            ...
            $stmtTonKho->execute([
                ...
                ':MaLoaiKho'    => $maLoaiKho,
                ':TrangThai'    => 1,
                ':GhiChu'       => 'Nhập kho từ phiếu ' . $phieu['SoHoaDon'],
                ':MaPhieuNhap'  => $phieu['SoHoaDon'],
                ':MaHangHoa'    => $item['MaHangHoa']
            ]);
            ...
            $sqlUpdateKe = "
                UPDATE kehanghoa
                SET DaChua = :SoLuongNhap
                WHERE MaKeHang = :MaKeHang
            ";

            $stmtUpdateKe = $conn->prepare($sqlUpdateKe);
            $stmtUpdateKe->execute([
                ':SoLuongNhap' => $soLuongNhap,
                ':MaKeHang'    => $ke['MaKeHang']
            ]);
        }
        // ================= TONKHO_CHITIET =================
        $sqlInsertCT = "
        INSERT INTO tonkho_chitiet
            (MaTonKho, MaHangHoa, ThoiGianNhap, NgaySanXuat, HanSuDung, SoLuongTon, TrangThai)
        VALUES
            (:MaTonKho, :MaHangHoa, :ThoiGianNhap, :NgaySanXuat, :HanSuDung, :SoLuongTon, 1)
        ";
        $stmtInsertCT = $conn->prepare($sqlInsertCT);

       $stmtInsertCT->execute([
            ':MaTonKho'     => $maTonKho,
            ':MaHangHoa'    => $maHangHoa,
            ':ThoiGianNhap' => $phieu['ThoiGian'], // từ phieunhap
        ...
        unset($_SESSION['PhieuNhap_items']);
        unset($_SESSION['Nhapkho_items']);
        ...
        $conn->rollBack();
        echo "Lỗi lưu phiếu nhập: " . $e->getMessage();
    }
}
...
<div class="layout">
    ...
    <div class="content">
        <div class="form-container">
            <div class="form-title" style="color: #007b36">PHIẾU NHẬP KHO</div>
        <form method="post" id="formHangHoa">
            <div class="section-box">
                ...
                <div class="form-group">
                    <div class="form-field">
                        ...
                        <input type="text" id="sohoadon"  name="sohoadon"placeholder="PN001"  value="<?= $phieu['SoHoaDon'] ?? $soHoaDon ?>" readonly>
                    </div>
                    <div class="form-field">
                        <label for="nguoinhanhang">Người giao hàng</label>
                        <input type="text" id="nguoigiaohang" name="nguoigiaohang" value="<?= $phieu['NguoiGiaoHang'] ?? '' ?>" placeholder="Nguyễn Văn A" >
                    </div>
                    <div class="form-field">
                        <label for="donvi">Đơn vị</label>
                        <input type="text" id="donvi" name="donvi" value="<?= $phieu['DonVi'] ?? 'Phòng tài chính' ?>" style="width:100px;" readonly>
                    </div>
                    <div class="form-field">
                    ...
                    <div class="form-field">
                        ...
                        <input type="text" id="bophan"  name="bophan" value="<?= $phieu['BoPhan'] ?? 'Nhập kho' ?>" style="width:100px;"placeholder="Nhập kho" readonly>
                    </div>
                    <?php
                    ...
                    <div class="form-field">
                        ...
                        <select id="nhaptaikho" name="nhaptaikho" class="form-select">
                            <option value="">-- Chọn loại kho --</option>

                            <?php foreach ($dsLoaiKho as $kho): ?>
                                <option 
                                    value="<?= $kho['TenLoaiKho'] ?>"
                                    <?= (isset($phieu['NhapTaiKho']) && $phieu['NhapTaiKho'] == $kho['TenLoaiKho']) ? 'selected' : '' ?>
                                >
                    ...
                    <div class="form-field">
                        <label for="diadiem">Địa điểm kho</label>
                ...
                <div class="section-title">Thông tin hàng hóa</div>
                    <div class="form-group">
                        <div class="form-field">
                            ...
                            <label for="succhua_view">Sức chứa hiện tại</label>
                            <input type="text" id="succhua_view" readonly >
                        </div>
                        <div class="form-field">
                        ...
                        <div class="form-field">
                            ...
                            <?php foreach ($dsHangHoa as $hh): ?>
                                <?php $succhua = laySucChua($conn, $hh['MaHangHoa']); ?>
                        ...
                        document.getElementById("hanghoa").addEventListener("change", function(){
                            ...
                            let mahh = option.value;
                            let succhua = option.dataset.succhua;

                            document.getElementById("mahanghoa_view").value = mahh;
                            document.getElementById("succhua_view").value = succhua;

                        });

                        </script>
                        <div class="form-field">
                            <label for="soluongthuctenhap">Số lượng thực tế</label>
                            <input type="number" id="soluongthuctenhap"  min="0"name="soluongthuctenhap"placeholder="5">
                        </div>
                        <script>
                            function closePopup() {
                            ...
                            document.getElementById("formHangHoa").onsubmit = function(e) {
                                //ràng buộc số lượng thực tế phải nhỏ hơn hoặc bằng số lượng theo chứng từ
                                    ...
                                    if (document.getElementById("nguoigiaohang").value.trim() === "") {
                                         e.preventDefault(); // CHẶN LỆNH GỬI FORM TẠI ĐÂY
                                        showPopup("Người giao hàng không được để trống");
                                        return false;
                                    }
                                    ...
                                    if (document.getElementById("nhaptaikho").value.trim() === "") {
                                         e.preventDefault(); // CHẶN LỆNH GỬI FORM TẠI ĐÂY
                                        showPopup("Nhập tại kho không được để trống");
                                        return false;
                                    }
            ...
            <div class="data-table-container">
                <table class="data-table">
                    ...
                    <tbody>
                        <?php foreach ($_SESSION['Nhapkho_items'] as $i => $item): ?>
                            <tr class="clickable-row"
                                ...
                                data-sltte="<?= $item['SoLuongThucTeNhap'] ?>"
                            ...
                            document.addEventListener("DOMContentLoaded", function () {
                                document.querySelectorAll(".clickable-row").forEach(function(row) {
                                    row.addEventListener("click", function() {
                                        ...
                                        document.getElementById("soluongthuctenhap").value = this.dataset.sltte;
            ...
            <div class="summary-and-actions">
                <div class="summary-info">
                    <div class="summary-field">
                        <?php
                        $tongSo = count($_SESSION['Nhapkho_items']);
                        $tongTien = 0;
                        foreach($_SESSION['Nhapkho_items'] as $item){
