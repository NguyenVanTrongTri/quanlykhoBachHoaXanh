...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['export_invoice'])) {

    $maPhieuNhap = $_POST['maphieunhap'] ?? '';

    if (empty($maPhieuNhap)) {
        $_SESSION['popup_error'] = true;
        $_SESSION['popup_message'] = "Vui lòng chọn phiếu nhập trước !";
        header("Location: " . $_SERVER['PHP_SELF']);
        exit;
    }
    header("Location: XuatHoaDonNhap.php?maphieunhap=" . urlencode($maPhieuNhap));
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['edit_item'])) {
    ...
    // Kiểm tra khóa chính
    if (empty( $maPhieuNhap)) {
        echo "Lỗi: không có mã phiếu nhap!";
        exit;
    }
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['delete_item'])) {
    ...
    // Kiểm tra
    if (empty($maPhieuNhap)) {
        echo "<script>alert('Lỗi: Không có mã phiếu nhập để xóa!');</script>";
        exit;
    }
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['search_item'])) {
    ...
    if ($keyword !== '') {
        $sql = "SELECT * FROM phieunhap
                WHERE MaPhieuNhap LIKE :keyword
                   OR DonVi LIKE :keyword
                   OR BoPhan LIKE :keyword
                   OR NguoiGiaoHang LIKE :keyword";
        $stmt = $conn->prepare($sql);
        ...
        <div class="form-container">
            ...
            <form method="post">
                ...
                <div class="form-group">
                    <div class="form-field">
                        ...
                        <input type="text" id="maphieunhap" name="maphieunhap"placeholder="">
                    </div>
                    <div class="form-field">
                        <label for="nguoigiaohang">Người giao hàng</label>
                        <input type="text" id="nguoigiaohang" name="nguoigiaohang" placeholder="">
                    </div>
                    ...
                    <div class="form-field">
                        ...
                        <input type="datetime-local" id="thoigian" name="thoigian" placeholder="">
                    </div>
                    <div class="form-field">
                        <label for="nhaptaikho">Nhập tại kho</label>
                        <input type="text" id="nhaptaikho" name="nhaptaikho" placeholder="">
                    </div>
                ...
                <div class="action-buttons">
                    ...
                    <button type="submit" name="export_invoice" class="btn clear">Xuất hóa đơn</button>
                </div>
            </form>
            <div class="data-table-container">
                <table class="data-table">
                   ...
                   <tbody>
                        <?php
                        $data = !empty($searchResults) ? $searchResults : getPhieuNhap();
                        $stt = 1;
                        foreach ($data as $row) {
